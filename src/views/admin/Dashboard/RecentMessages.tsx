...
function Message({ image, firstname, lastname, message }: MessageProps) {
  return (
    <div className="flex my-4 justify-start">
      {/* Name and Image */}
      <div className="w-1/5 inline-flex flex-col justify-start items-center text-slate-900">
        <img
          src={image}
          alt="seller image"
      ...
      {/* Message */}
      <div className="w-4/5 inline-flex ml-4 h-[5rem] bg-slate-900 rounded-2xl py-2 px-3 text-white overflow-hidden text-sm">
        <span>{message}</span>
      </div>
    </div>
  );
  ...
  return (
    <div className="w-full lg:flex-1 bg-white/5 rounded-2xl px-4 py-2">
      <div className="text-white font-semibold flex justify-between">
        ...
        <Link to="">View all</Link> {/* TODO: implement View All function */}
      </div>
